...
const dynamicImport = async () => {
  import('https://esm.sh/canvas-confetti@1.6.0').then((module) => {
    module.default({
      particleCount: 100,
      spread: 70,
      origin: { y: 0.6 }
...
const FeatureCard = ({ iconSvg, name, desc }: any) => {
  return (
    <Box
      ...
      rounded='$md'
    >
      <Button onPress={dynamicImport}>Import and Run</Button>
    </Box>
  )
}
...
const Container = () => {
  return (
    <Box flex={1} bg='$black' h='100vh' onMouseMove={onMouseMove}>
      <Box
        position='absolute'
        sx={{
          '@base': {
